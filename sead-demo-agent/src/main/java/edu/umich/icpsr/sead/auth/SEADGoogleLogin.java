	...
	public static String getAccessToken() {
		...
		if (refresh_token == null) {
			...
			if (refresh_token != null) {
				...
				try {
					writer = new PrintWriter("/home/sead/refresh.txt", "UTF-8");
					writer.print(refresh_token);

				} catch (FileNotFoundException e) {
					log.error("Could not write /home/sead/refresh.txt: "
							+ e.getMessage());
	...
	private static void initGProps() {
		...
		try {
			gProps = mapper.configure(DeserializationFeature.UNWRAP_ROOT_VALUE,
					true).readValue(
					SEADGoogleLogin.class.getClassLoader().getResourceAsStream(
							"sead-google.json"), GoogleProps.class);
		} catch (Exception e) {
