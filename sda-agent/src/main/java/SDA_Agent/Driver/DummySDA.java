...
public class DummySDA {
	...
	public DummySDA(PropertiesReader properties, JSONObject ore){
	...
	public void writeOREmap(String rootPath, JSONObject ore){
		try{
			FileWriter outFile = new FileWriter(rootPath +File.separator+"OREmap.txt", true);
			try {
			    PrintWriter out1 = new PrintWriter(outFile);
			    try {
			        out1.append(ore.toJSONString());
			    } finally {
			       out1.close();
			    }
			} finally {
